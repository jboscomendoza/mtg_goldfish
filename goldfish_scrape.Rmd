...
Empeceos la exploración con los estadísticos descriptivos más elementales. La media, mediana estándar y máximo y mínimo de los precios. La desviación estándar, un estadístico descriptivo por excelencia, en este caso es poco informativo, debido a la forma en que se distribuyen los precios.

Usamos las funciones `group_by()` y `summarize()` de *dplyr*, para aplicar las funciones `mean()`, `median()`, `min()` y `max()` por Rareza.
